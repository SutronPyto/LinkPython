...
Sample code:
   ...
   instrument = minimalmodbus.Instrument('RS232', 1, minimalmodbus.MODE_ASCII)
   instrument.write_register(24, 12, 1)
...
class Instrument():
    ...
    def read_floats(self, registeraddress, numberOfFloats,
                    functioncode=3, numberOfRegistersPerFloat=2):
        """
        Read multiple floating point numbers from consecutive registers.
        ...
        _checkInt(numberOfFloats,
                  minvalue=1,
                  description='number of floats')

        _checkInt(numberOfRegistersPerFloat,
                  minvalue=2,
                  maxvalue=4,
                  description='number of registers per float')

        return self._genericCommand(
            functioncode,
            registeraddress,
            numberOfRegisters=numberOfFloats *
                              numberOfRegistersPerFloat,
            payloadformat='floats'
        )
    ...
    def _genericCommand(self, functioncode, registeraddress, value=None, \
        ...
        PAYLOADFORMAT_FLOATS    = 'floats'
        PAYLOADFORMAT_STRING    = 'string'
        PAYLOADFORMAT_REGISTER  = 'register'
        PAYLOADFORMAT_REGISTERS = 'registers'

        ALL_PAYLOADFORMATS = [PAYLOADFORMAT_LONG, PAYLOADFORMAT_FLOAT, PAYLOADFORMAT_FLOATS,\
        ...
        if functioncode in [3, 4]:
            ...
            elif payloadformat == PAYLOADFORMAT_FLOATS:

                values = []

                for i in range(0, len(registerdata), 4):
                    floatbytes = registerdata[i:i + 4]
    ...
    def _communicate(self, request, number_of_bytes_to_read):
        ...
        if sys.version_info[0] > 2:
            request = str_to_bytes(request)  # Convert types to make it Python3 compatible
        ...
        if sys.version_info[0] > 2:
            answer = bytes_to_str(answer)  # Convert types to make it Python3 compatible
...
def _pack(formatstring, value):
    ...
    if sys.version_info[0] > 2:
        return bytes_to_str(result)  # Convert types to make it Python3 compatible
    return result
...
def _unpack(formatstring, packed):
    ...
    if sys.version_info[0] > 2:
        packed = str_to_bytes(packed)  # Convert types to make it Python3 compatible
...
def _hexdecode(hexstring):
    ...
    if sys.version_info[0] > 2:
        by = str_to_bytes(hexstring)
        try:
            return bytes_to_str(binascii.unhexlify(by))
...
@MEASUREMENT
def read_RDE300i_PVtemp(arg):
    # single measurement
    rde300i = Instrument('RS485', 30, MODE_RTU)
    temp = rde300i.read_float(1025,4, 2)

    return temp

@TXFORMAT
def read_IV_add_to_TX(standard):
    # read all IV curve values

    # time stamp
    ts = time.localtime()
    strftime = "{:02d}/{:02d}/{:4d},{:02d}:{:02d}:{:02d},".format(ts[1], ts[2], ts[0], ts[3], ts[4], ts[5])

    rde300i = Instrument('RS485', 30, MODE_RTU)
    # read in increment of 50 values
    v1 = rde300i.read_floats(1123,50, functioncode=4, numberOfRegistersPerFloat=2)
    c1 = rde300i.read_floats(1323, 50, functioncode=4, numberOfRegistersPerFloat=2)
    v2 = rde300i.read_floats(1223, 50, functioncode=4, numberOfRegistersPerFloat=2)
    c2 = rde300i.read_floats(1423, 50, functioncode=4, numberOfRegistersPerFloat=2)

    # format string
    iv_values = strftime + "IV_V," + ",".join("{:.2f}".format(x) for x in v1) +\
                ",".join("{:.2f}".format(x) for x in v2) + "\n" +\
                strftime + "IV_I," + ",".join("{:.2f}".format(x) for x in c1) +\
                ",".join("{:.2f}".format(x) for x in c2)

    # add values to TX
    return standard + iv_values

"""
ideas 2 do 
- function to store IV curve to SD card
    - a: into TX folder for transmission
    - b: only into data folder for storage 
"""