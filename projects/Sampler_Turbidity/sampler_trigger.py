""" Triggers ISCO sampler via SDI-12 command, based on a measurement, and the time of the last sample.
    Very similar to the sampler_turbidity.py script, but more general purpose, triggering on any measurement.

    To use script:
        1) Setup a measurement and connect it to the sampler_meas function
        2) Setup General Purpose Variables:
            GP1 is the Threshold (if measurement exceeds threshold, sampler may be triggered)
...
# measurement that triggered last sample
value_last_sample = 0.0
...
def status_update(measurement=True, value=0.0):
    ...
    :param value: if measurement, what the sensor reading was
    :type value: float
    :return: None
    """
    global samples_triggered
    ...
    if time_last_sample:
        ...
        if measurement:
            print("Trigger value: {}".format(value))
        else:
...
@MEASUREMENT
def sampler_meas(measurement_reading):
    """ Connect a measurement to this function to trigger
    the sampler based on sensor reading"""
    sample_now = False
    ...
    if threshold <= 0:
        ...
        if measurement_reading > threshold:  # threshold is exceeded
            if time_last_sample == 0:
                ...
                if min_time_interval < 0:
                    ...
                    sample_now = True
                    turbidity_last = measurement_reading

    if sample_now:
        trigger_sampler_low()

    status_update(True, measurement_reading)
    print("Last measurement_reading: {}, {}".format(measurement_reading, ascii_time(time_scheduled())))
